...
const EditDelete = ({ attributes, setAttributes, isBackend = false }) => {
    const { hotspotData = [], scenes } = attributes;
    ...
    console.log("Tour viewer is Loaded");
    ...
    useEffect(() => {
        ...
        scenes.map((scene) => {
            modifiedScenes[scene.id] = {
                ...scene,
                // yaw: scene.yaw + currentYaw,
            };
        })
        ...
        const viewer = window.pannellum.viewer(panoRef.current, {
            ...
            panorama: panoImg,
            autoLoad: true,
            hotSpots: hotspotData.map((spot, index) => ({
                pitch: spot.pitch,
                yaw: spot.yaw,
                type: "custom",
                text: spot.text,
                ...
                createTooltipFunc: (hotSpotDiv) => {
                    ...
                    icon.style.cssText = `
                        ...
                        color: white;
                    `;
                    icon.innerHTML = spot.type === 'scene' ? '↑' : 'i';
